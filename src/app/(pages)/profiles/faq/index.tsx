...
const FAQAccordion = () => {
    ...
    const renderHeader = (section: FAQItem,
        ...
        return (
            <View
                className={`flex-row justify-between items-center p-4 rounded-xl mb-2 bg-card dark:bg-card-dark border border-border dark:border-border-dark ${isActive ? 'border-primary' : ''}`}
            >
                <Text className='flex-1 mr-3 text-sm font-manrope-semibold text-text dark:text-text-dark'>
                    {section.title}
                </Text>
                <View className={`w-8 h-8 rounded-full items-center justify-center ${isActive ? 'bg-primary/10' : 'bg-input dark:bg-input-dark'}`}>
                    <Ionicons
    ...
    const renderContent = (section: FAQItem) => {
        return (
            <View className={`p-4 rounded-xl mb-3 mx-1 ${isDark ? 'bg-input-dark' : 'bg-input'} border ${isDark ? 'border-border-dark' : 'border-border'}`}>
                <Text className={`text-sm leading-6 font-manrope-regular ${isDark ? 'text-text-dark' : 'text-text'}`}>
                    {section.content}
                </Text>
            </View>
        );
    ...
    return (
        ...
        >
            ...
            >
                ...
                <View className="mt-6 px-4 py-4 bg-primary/5 dark:bg-primary/10 rounded-xl border border-primary/20">
                    <View className="flex-row items-center gap-3">
                ...
                {/* Footer */}
                <View className="mt-4 px-4 py-3 bg-input dark:bg-input-dark rounded-xl">
