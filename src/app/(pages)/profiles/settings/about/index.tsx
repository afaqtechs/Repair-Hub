    ...
    >
      ...
      >
        ...
        <View className="items-center mb-8">
          <View className="w-24 h-24 bg-primary/10 dark:bg-primary/20 rounded-3xl items-center justify-center mb-4">
            <Ionicons name="build-outline" size={48} color="#6366F1" />
          </View>
          <Text className="text-2xl font-manrope-bold text-text dark:text-text-dark">
            Repair Hub
          </Text>
          <Text className="text-sm font-manrope-light text-text-muted dark:text-text-darkMuted mt-1">
        ...
        <View className="mb-6">
          ...
          <View className="bg-card dark:bg-card-dark px-5 py-4 border border-border dark:border-border-dark rounded-lg">
            {features.map((feature, index) => (
              <View
                key={index}
                className={`flex-row items-center py-2 ${index !== features.length - 1 ? 'border-b border-border/50 dark:border-border-dark/50' : ''}`}
              >
        ...
        <View className="mb-6">
          ...
          <View className="bg-card dark:bg-card-dark px-5 py-4 border border-border dark:border-border-dark rounded-lg">
            {teamMembers.map((member, index) => (
              <View
                key={index}
                className={`flex-row items-center justify-between py-2.5 ${index !== teamMembers.length - 1 ? 'border-b border-border/50 dark:border-border-dark/50' : ''}`}
              >
        ...
        <View className="mb-6">
          ...
          <View className="bg-card dark:bg-card-dark px-5 py-4 border border-border dark:border-border-dark rounded-lg">
            <View className="flex-row justify-between py-2 border-b border-border/50 dark:border-border-dark/50">
              <Text className="text-sm font-manrope-medium text-text-muted dark:text-text-darkMuted">
                Version
              </Text>
              <Text className="text-sm font-manrope-semibold text-text dark:text-text-dark">
                {appVersion}
        ...
        <View className="flex-row gap-3 mb-6">
          ...
          >
            <Ionicons name="globe-outline" size={20} color={isDark ? "#F8FAFC" : "#171A2B"} />
        ...
        <View className="mt-2 px-4 py-3 bg-input dark:bg-input-dark rounded-xl">
          <View className="flex-row items-center gap-2">
            ...
            <Text className="text-xs text-text-muted dark:text-text-darkMuted font-manrope-light flex-1 text-center">
